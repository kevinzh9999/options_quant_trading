...
**v1 新映射系统（2026-04-14 已证伪，放弃）：**
数据驱动 PnL-ranked bucket 映射替代 v2 手工分档的研究项目，经 IS/OOS 验证失败：IM IS 内就不 valid，IC IS valid 但 OOS 失效。bucket 边界对数据噪音敏感，不可泛化。
研究遗产：三位一体评估方法论（post-hoc 标注优于独立 backtest + IS/OOS 分离 + breakeven 阈值 0.985pt），已应用于后续 F1 等研究。详见 `memory/feedback_v1_postmortem.md`。
...
### 时间标准（重要！两套时间共存 — 2026-04-24 全量 TZ 无关化）

系统中存在两套时间标准，**混淆会导致严重bug**（如2026-04-08回测全天M=0、2026-04-23 signal_log 时区错误）。

**核心约定**：
- K 线/Tick 数据（`*_min`, `futures_tick`）存 **UTC**（TQ bar ns 戳自带绝对时间）
- 所有其他时间字段（信号/订单/快照/面板/跨进程 JSON/时间判断）存 **BJ (UTC+8)**
- BJ 时间统一通过 `_bj_now()` helper 生成 = `datetime.utcnow() + timedelta(hours=8)`
- **禁止直接使用 `datetime.now()`**（Japan/US/任何非中国时区 Mac 上会返回错误时区）

#### DB 字段时区表

| 库.表 | 字段 | 格式 | 时区 | 写入方 |
|---|---|---|---|---|
| `trading.futures_min` | `datetime` | `YYYY-MM-DD HH:MM:SS` | **UTC** | EOD DataDownloader |
| `trading.index_min` | `datetime` | 同上 | **UTC** | EOD |
| `options_data.options_min` | `datetime` | 同上 | **UTC** | EOD |
| `tick_data.futures_tick` | `datetime` | `YYYY-MM-DD HH:MM:SS.ffffff` | **UTC** | EOD |
| `etf_data.etf_min` | `datetime` | 同上 | **UTC** | EOD |
| `trading.signal_log` | `datetime` | `YYYY-MM-DD HH:MM:SS` | **BJ** | monitor (`current_time_bj`) |
| `trading.orderbook_snapshots` | `datetime` | 同上 | **BJ** | monitor |
| `trading.trade_decisions` | `datetime` / `created_at` | 同上 | **BJ** | monitor (`_bj_now()`) |
| `trading.shadow_trades` | `entry_time` / `exit_time` | `HH:MM` | **BJ** | monitor (`_now_bj`) |
| `trading.order_log` | `datetime` | `YYYY-MM-DD HH:MM:SS` | **BJ** | executor (`_bj_now()` / signal JSON ts) |
| `trading.executor_log` | `signal_time` / `receive_time` / `cancel_time` | 同上 | **BJ** | executor |
| `trading.account_snapshots` | `snapshot_time` | 同上 | **BJ** | EOD |
| `trading.trade_records` | `trade_time` | `HH:MM:SS` | **BJ** | EOD (TQ → Asia/Shanghai) |
| `trading.vol_monitor_snapshots` | `datetime` | 同上 | ⚠️ 未审计 | vol_monitor |
| **daily 表**（`futures_daily`/`index_daily`/`options_daily`/`daily_model_output`/`morning_briefing`/`account_snapshots.trade_date`/…） | `trade_date` / `list_date` / … | `YYYYMMDD` | 日期（BJ 日历日） | 各 EOD/briefing |

**"日期（BJ 日历日）" 说明**：字段本身只有年月日无时分秒，不存在时区概念。但生成时用 `_bj_now().strftime("%Y%m%d")` 确保在任何系统时区都按 BJ 日历切日（避免美国时区凌晨把今天写成昨天）。

#### 跨进程 JSON (tmp/)

| 文件 | 字段 | 时区 | 写入方 → 读取方 |
|---|---|---|---|
| `signal_pending_{sym}.json` | `timestamp` | BJ | monitor → executor |
| `futures_positions_{sym}.json` | `timestamp` | BJ | monitor → executor |
| `shadow_state_{sym}.json` | `trade_date` / `updated_at` | 日期 / BJ | monitor → monitor |
| `auto_lock_{sym}.json` | `timestamp` | BJ | executor → monitor |
| `denied_positions.json` | `date` / `deny_time` | 日期 / BJ | executor → monitor |

#### 代码中的时间判断

| 位置 | 逻辑 | 时区 |
|---|---|---|
| Monitor/Executor 主循环收盘检查 | `_bj_now().hour >= 15 and .minute >= 5` | **BJ** |
| Executor 信号过期（300s） | `_bj_now() - sig_dt` | BJ diff |
| Executor reconcile 新鲜度（600s） | `_bj_now() - ts_dt` | BJ diff |
| Executor reconcile stale-guard | `_bj_now().replace(hour=entry_t[:2]…)` | BJ |
| `_check_auto_stop_lock` 14:30 BJ 关断 | `datetime.utcnow().strftime("%H:%M") >= "06:30"` | **UTC**（常量也 UTC，刻意对称） |
| `_check_auto_locks` hold_minutes | `datetime.utcnow() - entry_utc` | **UTC**（两端都 UTC） |
| `is_open_allowed` 交易时段 | bar datetime 提 `%H:%M` | **UTC**（K 线本来就 UTC） |
| `_now_utc` / `_now_bj`（`_on_new_bar`） | `_bar_utc + 5min`, `_now_utc + 8h` | 从 bar 推导，TZ-independent |

#### 规则

1. **K 线/Tick 入库** → `pd.to_datetime(ns, unit="ns")` 自动 UTC
2. **运行时时间戳** → **必须**用 `_bj_now()`，不用 `datetime.now()`
3. **时间 diff / 比较** → 两端同时区（都 BJ 或都 UTC），`_bj_now()` 与 BJ 字段比
4. **显示面板** → `_bj_now().strftime(...)` 直接显示 BJ
5. **回测取 bar 时间** → `_get_utc_time(bar_5m)` 必须是 UTC 才能和 `NO_TRADE_BEFORE/AFTER` 比
6. **跨日期边界** → 用 `_bj_now().strftime("%Y%m%d")` 按 BJ 日历切日

#### 历史教训

- **2026-04-08**：EOD 重构引入 DataDownloader，CSV 输出 BJ 但入库未减 8h → index_min 存了 BJ → 回测 `_get_utc_time` 判断 09:30 > 07:00 超出交易窗口 → 全天 score_all 返回 None
- **2026-04-08 批量修复**：修 index_min UTC 时误伤 signal_log 等 BJ 表 → 信号记录时间错乱
- **2026-04-23 日本 Mac 时区**：monitor/executor 多处 `datetime.now()` 在 JST Mac 上返回 JST（UTC+9），不是 BJ。signal_log/orderbook/trade_decisions 长期写 UTC（来自 `current_time_utc`），order_log LOCK 和 JSON timestamp 又是 JST，三套时区混用
- **2026-04-24 全量修复**：引入 `_bj_now()` helper，monitor.py + order_executor.py 全部 `datetime.now()` 替换。历史数据（`datetime < '2026-04-24'`）保持原混乱格式，回测/分析跨修复点时按日期分支处理

#### 仍未审计（按需扩展）

- `scripts/vol_monitor.py`
- `scripts/quadrant_monitor.py`
- `scripts/morning_briefing.py`
- `dashboard/` 只读 DB，无写入 — 可以忽略

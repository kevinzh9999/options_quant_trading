...
**低振幅日IC为负的深层解读**：不是噪音，而是低振幅日动量信号就是反向的（均值回归占主导）。振幅过滤器本质上是regime切换。未来可探索：低振幅日用反向信号。
...
- 低振幅日用反向信号（均值回归）是理论上的最优策略，但实施复杂度高
...
### 阶段性结论：暂停优化，转入运营监控（2026-04-05）

**方案D + 当前因子结构是215天数据能支撑的最优配置。继续优化的过拟合风险大于收益。**
...
|--------|------|---------|
| P0 | 跑实盘，积累交易数据 | 立即 |
| P1 | 每周对比实盘PnL vs 回测预期(+20pt/天) | 持续 |

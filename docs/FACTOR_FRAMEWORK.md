...
> 版本：1.2 | 2026-04-08（更新：方案E动态lb成果——Phase 1低振幅IC发现的直接应用）
...
1. **前瞻偏差**：compute_series 不能用未来数���。forward_periods 的 shift(-n) 只在 evaluator 中用，因子本身不能 shift 负值
2. **存活偏差**：如果因子用到的数据（如期权行情）只有部分日期有，需要���注覆盖率
3. **过拟合**：一个因子如果在IC grid search中只有特定参数好，其他参数都很差→可能过拟合
4. **交易成本**：高换手率的因子（每根bar排名剧烈变化）在实盘中成本高
5. **和现有因子���关系**：新因子必须和M/V/Q的相关性 < 0.5 才有独立价值。之前验证VWAP(r=0.67)被排除就是这个原��
6. **IC自相关高估**：5分钟bar之间高度自相关（09:45和09:50因子值几乎一样），全局IC的统计显著性被严重高估。应同时看daily_ic（每天算一个IC，看均值和标准差），更接近实盘决策频率（一天2-3笔）
7. **vwap_cumulative跨日污染**：当前实现是全局cumsum，没有按日重置。Alpha041等用到VWAP的因子在回测时会跨日污染。需要按交易日分组重置（TODO）
8. **101 Alphas是截面选股因子**：原始论文针对美股截面设计，预测力来自不同股票间的相对强弱。用在单品种时间序列上效果可能差异大，当作"灵感来源"而非"已验证有效因子"
9. **Phase 3回测需要样本内/外分离**：不能用215天同时找最优因子和最优打分映射阈值。建议前150天训练映射，后65天做样本外验证

### 5.5 实盘集成说明

**Phase 1-3 只用于离线研究，不接入实盘 monitor。** 基类 `compute()` 的默认实现是每次算完整时间序列再取最后值，实盘性能不可接受。接入实盘是 Phase 4+ 的事，需要：
- 子类覆盖 `compute()` 做增量计算
- 或实现滑动窗口版本
- 充分测试后才接入 monitor
...
### Phase 1：建框架 + 评估M分替代 ✅ 完成（2026-04-05）

```
✅ 创建 models/factors/ 目录结构
✅ 实现 operators.py（30+算子：时间序列/加权/截面/数学/K线结构）
✅ 实现 base.py（Factor基类）
✅ 实现 catalog_price.py（7个M分候选 + 4个参数变体 = 11因子）
✅ 实现 evaluator.py（IC + IC_IR + 分组收益 + 单调性 + 振幅regime分组）
✅ 跑 M分 11个候选的评估报告（10295根5分钟bar）
✅ scripts/factor_research.py 研究脚本
```

**Phase 1 M分评估结果：**

| 因子 | IC_3bar | IC_5bar | 单调性 | Regime: 低振幅 | Regime: 高振幅 |
|------|---------|---------|--------|--------------|--------------|
| **mom_rank_48** | **+0.043** | +0.036 | **0.90★** | -0.14 | **+0.13** |
| mom_ema_5_20 | +0.036 | +0.031 | 0.60 | -0.15 | +0.12 |
| mom_simple_18 | +0.032 | +0.031 | 0.60 | -0.14 | +0.11 |
| mom_decay_12 | +0.026 | +0.010 | 0.40 | -0.15 | +0.13 |
| **mom_simple_12★baseline** | **+0.012** | +0.000 | 0.70 | -0.15 | +0.10 |

**关键发现：**
1. baseline `mom_simple_12` IC排倒数第三（0.012），`mom_rank_48` IC是其3.5倍（0.043）
2. **低振幅日所有动量因子IC为负（-0.10~-0.15）**——低振幅日做动量是反向的！
3. **高振幅日所有因子IC为正（+0.09~+0.13）**——完美解释振幅过滤器的有效性
4. `mom_rank_48` 是唯一单调性显著的因子（corr=0.90, p=0.037）
5. 所有M分候选相关性>0.5（最低0.52），替换后改善受限于高共线性
6. Q1→Q5分组收益（3bar bps）：-0.1, -0.5, +0.7, +1.1, +2.4 完美单调

**方法论注意**：上述IC是全局IC（10295根bar做Spearman），因bar间自相关可能高估显著性。Daily IC（每天算一个IC）更稳健——已在evaluator中加入。

**低振幅日IC为负的深层解读**：不是噪音，而是低振幅日动量信号就是反向的（均值回归占主导）。振幅过滤器本质上是regime切换。**已在方案E中实施**：低振幅日用lb=4（更短lookback适应快速反转），高振幅日用lb=12（捕捉持续趋势），+20.2%改善。

**M分边际贡献有限**：所有候选高度共线(>0.5)，本质度量同一个东西。真正alpha来源更多在V/Q/B叠加和regime过滤。

### Phase 1-2 综合结论（2026-04-05）

**核心发现：**

1. **Daily IC和Global IC差距巨大（自相关膨胀）——必须以Daily IC为主**
   - Global IC因bar间自相关严重高估：mom_rank_48全局IC=+0.043，Daily IC=**-0.204**（负的！）
   - 全部M分因子Daily IC为负（-0.05~-0.24），方向性预测力实为负值
   - 唯一正Daily IC的因子类别是V分（vol_atr_ratio dIC=+0.065）

2. **V分（vol_atr_ratio）是唯一持续正Daily IC的因子，无需替换**
   - baseline vol_atr_ratio_5_40 dIC_3=+0.065, dICIR=+0.27，已是V分类最优
   - V分因子间相关性低（0.10~0.73），有独立信息但baseline已最优

3. **M分和B分Daily IC为负——赚钱靠V/Q调节和regime过滤，不是靠M分预测方向**
   - M分: dIC=-0.12~-0.24（全部负）
   - B分: price_pos dIC=-0.30, boll_breakout dIC=-0.16（全部负）
   - 策略的盈利机制：V/Q提供正信号 → M/B提供方向（虽然IC负，但经乘数链过滤后净效果为正）

4. **ts_rank(close, N)整体优于simple return——rank标准化有价值**
   - 回测验证：rank_24~72全部优于baseline（+130~+901pt）
   - 但lookback=48是**尖峰**（42=+276, 48=+901, 54=+596），典型过拟合
   - 时间分段：前半+824，后半仅+77——优势不稳定
   - **不宜精确定参实施**

5. **101 Alphas在单品种时间序列上基本无效**
   - alpha041 dIC=-0.354（极负），alpha001 dIC=+0.035（弱）
   - 原始论文针对截面选股，用在单品种时序效果差

6. **Q分弱但稳定**
   - qty_ratio_20 dIC=+0.034, qty_trend dIC=+0.027（弱正）
   - qty_price_corr dIC≈0（无效）
   - qty_signed_flow regime IC有意思（低振幅-0.09，高振幅+0.10），但全局IC不稳定

7. **Regime IC pattern一致：低振幅日M/B因子全部反向**
   - 低振幅日：M dIC=-0.15, B dIC=-0.15（均值回归占主导）
   - 高振幅日：M dIC=+0.13, B dIC=+0.14（动量有效）
   - V分：高振幅日IC反而为负（-0.08）——高振幅时波动率因子过饱和

**待办：**
- 积累更多数据后（6个月+），重新评估ts_rank的lookback稳健区间
- 考虑把ts_rank作为M分的辅助因子（而非替代），叠加而非替换
- V分增强是更有前景的方向（唯一正Daily IC），探索波动率regime的进一步利用
- ~~低振幅日用反向信号（均值回归）是理论上的最优策略，但实施复杂度高~~ → **已通过方案E动态lb部分实现**（低振幅日lb=4更快响应反转，+20.2%改善）

### Phase 2：评估V/Q/B替代 + 101 Alphas ✅ 完成（2026-04-05）
...
### 阶段性结论：暂停优化，转入运营监控（2026-04-05，04-08更新）

**方案E（动态lb）+ 当前因子结构是216天数据能支撑的最优配置。继续优化的过拟合风险大于收益。**

Phase 1-2的核心价值不是"找到更好的因子"，而是**理解了策略为什么赚钱**：
- 不是因为M分能预测方向（Daily IC为负）
- 而是V/Q提供正信号 + regime过滤（振幅）去掉了亏钱环境 + 乘数链控制了风险

**方案E动态lb（2026-04-07~08）**：Phase 1发现"低振幅日M分IC为负"直接催生了动态lb方案：
- 低振幅日（<1.5%）用lb=4（20分钟），高振幅日用lb=12（60分钟）
- 216天回测：IM+2120 IC+2836 = **+4956pt**（+20.2% vs 方案D +4124pt）
- 稳健性：时间分半前+21%后+30%，12个邻域全正，逐月无亏损
- 这是Phase 1"低振幅日动量因子IC为负"发现的直接应用成果
...
|--------|------|---------|
| P0 | 跑实盘，积累交易数据 | 立即 |
| P1 | 每周对比实盘PnL vs 回测预期(+23pt/天) | 持续 |
| P2 | 重新验证ts_rank lookback | 数据到400+天（约2026年8月） |
| P3 | V分增强研究 | 数据到400+天 |
| P4 | 低振幅日均值回归策略 | 有带宽时，全新策略设计 |

**实盘退化判断标准：**
- 连续4周均PnL < +5pt/天 → 检查市场regime是否变化
- 连续2月合计亏损 → 回顾参数是否需要重新校准
- 以优化前的+3.5pt/天（216天保守基线）作为底线

### 打分映射sensitivity分析（2026-04-05）

**连续化打分 vs 阶梯式：阶梯式全面更优。**
- 纯线性M分: -221pt，M+V+Q全线性: -621pt
- 阶梯式的硬断点是天然的噪音过滤器，连续化放行了低质量信号

**M/V/Q断点和分值微调（215天全量）：**

| 维度 | 当前最优？ | 最大改善 | 最优配置 | 稳健性 | 实施？ |
|------|-----------|---------|---------|--------|--------|
| M断点 | 近最优 | +140pt | 0.12/0.22/0.32 | 高原区 | 不改（噪音范围） |
| M分值 | 近最优 | +117pt | 15/30/40 | 中等 | 不改 |
| V断点 | **最优** | +61pt | 收紧上限1.3 | 非常稳健 | 不改 |
| V分值 | **最优** | 0 | 当前已最优 | 非常稳健 | 不改 |
| Q分 | 可微调 | +158pt | 1.2/0.3阈值 | 待验证 | 不改（无样本外） |

**关键发现：**
- 二值M分（0/0/35）: -470pt，中间档15/25分有实际价值
- 缩量Q分0分: -918pt，缩量环境给分比不给好得多
- V分打分是最稳健的（唯一正Daily IC的因子，打分配置也最优）
- 所有改善<160pt，在215天单日波动的噪音范围内，无样本外验证不实施

**决策：保持当前打分映射不变，等实盘4周数据后再评估。**

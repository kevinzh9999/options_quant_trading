...
| # | 问题 | 严重程度 | 影响 |
|---|------|---------|------|
| D13 | 止损用bar close判断而非high/low | 中等 | 乐观偏差：可能漏掉盘中触发的止损，高估P&L |
...
A1, A2, A3, B4, B5, B6, B7, C8, C9, C10, D11, D12, E14, E15, E16, F18

---

## 修复建议优先级

1. **D13（止损）**：高优先级。当前backtest的止损是"乐观止损"——如果bar内低点触及止损位但收盘价回升，backtest不会止损。实盘一定会被止损。修复方案：在check_exit前先用bar_T的high/low检查是否穿过止损位。
2. **F19/F20（价格来源）**：中优先级。需要长期积累期货分钟K线后，backtest改用期货价格做P&L计算。
3. **D13-NOTE（同bar极值矛盾）**：低优先级。出现频率低，影响小。

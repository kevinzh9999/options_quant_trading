...
### 1.8 Q分跨日偏差发现（2026-04-08，重要）

**问题**：回测和实盘的Q分（成交量维度）在开盘时段存在系统性偏差。

**根因**：
- 回测的`_score_volume`用rolling 20-bar均值，包含昨天尾盘的低量bar → 开盘放量ratio=3-8x → Q=20
- 实盘monitor的TQ现货K线(SSE.000852)在开盘时段volume疑似为0 → Q=0
- 从signal_log验证：4/7和4/8的开盘30分钟Q全是0，直到~10:05才出现Q=10

**影响**：
- 回测在09:40就能触发信号（M=50+Q=20=70 → 57>55），实盘要等到10:05+（V分也起来后）
- 回测首次入场比shadow早10-25分钟（系统性偏差，非个别日）
- 回测结果偏乐观：开盘30分钟的交易在实盘中不会发生

**方案B测试（同时段历史均量替代rolling 20-bar）**：
- IM -1111pt(-38%)，IC +90pt(+3%)，合计-1021pt(-18%)
- IM大幅恶化因为开盘假Q分信号被去掉，但这些信号实盘本来就拿不到
- **已revert**——方案B改变了Q分的fundamental逻辑，影响面过大

**当前处理**：回测中开盘前N根bar Q分强制为0（模拟实盘行为），消除偏差但不改Q分算法

**待办（明天盘中验证）**：
- 检查TQ的SSE.000852 5分钟K线的volume字段值
- 确认是TQ数据问题还是monitor代码问题
- 如果TQ现货volume正常，则monitor的Q计算有bug需要修复

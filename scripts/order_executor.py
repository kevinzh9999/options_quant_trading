...
# ---------------------------------------------------------------------------
# 自动bar_low锁仓（精度止损，绕开check_exit的5分钟bar延迟）
# ---------------------------------------------------------------------------
AUTO_LOCK_ENABLED_SYMBOLS = {"IM", "IC"}
AUTO_LOCK_SL_PCT = {"IM": 0.003, "IC": 0.003}  # 与SYMBOL_PROFILES.stop_loss_pct保持一致
AUTO_LOCK_AGGRESSIVE_OFFSET = 1.0              # 初始挂单 bid1-1 / ask1+1
AUTO_LOCK_FILL_TIMEOUT = 5.0                   # 初始挂单等待秒数，超时改追价
AUTO_LOCK_CHASE_OFFSET = 3.0                   # 追价偏移 ±3pt
AUTO_LOCK_DISABLE_AFTER_UTC = "06:30"          # 14:30 BJ 之后停用（monitor EOD接管）
...
def _execute_order(
    ...
    if filled > 0:
        if action == "OPEN":
            # 自动锁仓止损价（bar_low精度，绕开check_exit 5min延迟）
            sl_pct = AUTO_LOCK_SL_PCT.get(sym, 0.005)
            if direction == "LONG":
                stop_price = trade_price * (1 - sl_pct)
            else:
                stop_price = trade_price * (1 + sl_pct)
            positions[sym] = {
...
def _write_auto_lock_file(sym: str, payload: dict):
    """写 tmp/auto_lock_{sym}.json 供 monitor 读取，清理 shadow 持仓。"""
    try:
        path = os.path.join(_TMP_DIR, f"auto_lock_{sym}.json")
        with open(path, "w") as f:
            json.dump(payload, f, ensure_ascii=False)
    except Exception as e:
        print(f"  [WARN] 写 auto_lock 文件失败: {e}")


def _check_auto_stop_lock(positions: dict, tq_api, db_path: str, dry_run: bool):
    """每个主循环tick调用：检查持仓是否触达stop_price，触发立即锁仓。

    bar_low精度 — 不等5分钟bar收盘，tick级触发。
    用反向OPEN锁仓（避开平今10x手续费），次日开盘后手工平昨。
    """
    if tq_api is None:  # dry-run 或 未连接
        return

    # 14:30 BJ（UTC 06:30）之后停用，由 monitor EOD 接管
    try:
        utc_hm = datetime.utcnow().strftime("%H:%M")
        if utc_hm >= AUTO_LOCK_DISABLE_AFTER_UTC:
            return
    except Exception:
        pass

    for sym, pos in list(positions.items()):
        if sym not in AUTO_LOCK_ENABLED_SYMBOLS:
            continue
        if pos.get("auto_lock_sent"):
            continue
        contract = pos.get("contract", "")
        stop_price = pos.get("stop_price", 0)
        if not contract or not stop_price:
            continue

        try:
            quote = tq_api.get_quote(contract)
            last = float(getattr(quote, "last_price", 0) or 0)
            if last <= 0 or last != last:  # 0 或 NaN
                continue
        except Exception:
            continue
        ...
        triggered = False
        if direction == "LONG" and last <= stop_price:
            triggered = True
        elif direction == "SHORT" and last >= stop_price:
            triggered = True
        if not triggered:
            continue
        ...
        try:
            if direction == "LONG":
                init_px = float(quote.bid_price1) - AUTO_LOCK_AGGRESSIVE_OFFSET
            else:
                init_px = float(quote.ask_price1) + AUTO_LOCK_AGGRESSIVE_OFFSET
        except Exception:
            init_px = last

        print(f"\n  ⚠ AUTO_STOP_LOCK {sym} {direction} "
              f"last={last:.1f} stop={stop_price:.1f} "
              f"→ 锁仓 {tq_dir} {lots}手 @ {init_px:.1f}")

        # 立刻标记，防止主循环重入（下单前占坑）
        pos["auto_lock_sent"] = True

        if dry_run:
            print(f"  [DRY] 跳过真实下单")
            positions.pop(sym, None)
            _write_auto_lock_file(sym, {
                "sym": sym, "contract": contract, "direction": direction,
        ...
        trade_px = 0.0
        try:
            order = tq_api.insert_order(
                contract, direction=tq_dir, offset="OPEN",
                volume=lots, limit_price=init_px,
            )
            deadline = time.time() + AUTO_LOCK_FILL_TIMEOUT
            while not order.is_dead:
                tq_api.wait_update(deadline=deadline)
                if time.time() > deadline:
                    break
            filled = order.volume_orign - order.volume_left
            trade_px = float(getattr(order, "trade_price", init_px) or init_px)

            # 未成交 → 追价
            if order.volume_left > 0:
                if not order.is_dead:
                    tq_api.cancel_order(order)
                    cancel_dl = time.time() + 3
                    while not order.is_dead:
                        tq_api.wait_update(deadline=cancel_dl)
                        if time.time() > cancel_dl:
                            break
                remaining = order.volume_left
                if remaining > 0:
                    try:
                        quote2 = tq_api.get_quote(contract)
                        if direction == "LONG":
                            chase_px = float(quote2.bid_price1) - AUTO_LOCK_CHASE_OFFSET
                        else:
                            chase_px = float(quote2.ask_price1) + AUTO_LOCK_CHASE_OFFSET
                    except Exception:
                        chase_px = init_px - AUTO_LOCK_CHASE_OFFSET if direction == "LONG" else init_px + AUTO_LOCK_CHASE_OFFSET
                    print(f"  追价锁仓 {tq_dir} {remaining}手 @ {chase_px:.1f}")
                    order2 = tq_api.insert_order(
                        contract, direction=tq_dir, offset="OPEN",
                        volume=remaining, limit_price=chase_px,
                    )
                    dl2 = time.time() + AUTO_LOCK_FILL_TIMEOUT
                    while not order2.is_dead:
                        tq_api.wait_update(deadline=dl2)
                        if time.time() > dl2:
                            break
                    filled2 = order2.volume_orign - order2.volume_left
                    filled += filled2
                    if filled2 > 0:
                        trade_px = float(getattr(order2, "trade_price", chase_px) or chase_px)
        except Exception as e:
        ...
        if filled > 0:
            ...
            _record_order(db_path, {
                "datetime": ts, "symbol": sym, "direction": direction,
                "action": "LOCK", "limit_price": trade_px, "lots": filled,
                "filled_lots": filled, "filled_price": trade_px,
                "status": "FILLED", "signal_score": 0,
                "reason": "AUTO_STOP_LOCK",
            })
            # 通知 monitor 清理 shadow
            _write_auto_lock_file(sym, {
                "sym": sym, "contract": contract, "direction": direction,
                "lots": filled, "entry_price": entry,
                "exit_price": trade_px, "stop_price": stop_price,
                "reason": "AUTO_STOP_LOCK",
                "timestamp": ts,
            })
            # 从 executor 内部持仓移除，防止后续 CLOSE 信号再次下单
            positions.pop(sym, None)
            print(f"  锁仓成交 {filled}/{lots}手 @ {trade_px:.1f}")
        else:
            # 未成交保留 auto_lock_sent=True 避免下一tick立刻重试；
            # 但若想允许后续再试可改回 False。此处保守：等待人工介入。
            print(f"  [WARN] 锁仓未成交，已挂 auto_lock_sent=True 阻止重入，请手工处理")
...
def _reconcile_positions(positions: dict):
    ...
    for sym, info in tq_positions.items():
        ...
        if sym in positions:
            ...
            # 回填 contract（restore 时为空）
            if sym in positions and not positions[sym].get("contract") and contract_tq:
                positions[sym]["contract"] = contract_tq
        elif net_tq != 0:
...
def _restore_positions_from_log(db_path: str, positions: dict):
    ...
    for sym, info in net.items():
        ...
        positions[sym] = {
            ...
            "entry_price": entry,
            "entry_time": "restored",
            "contract": "",             # filled by _reconcile_positions next
            "stop_price": stop_price,
            "auto_lock_sent": False,
        }
        d_cn = "多" if direction == "LONG" else "空"
        restored.append(f"{sym} {d_cn}{lots}手@{entry:.0f}")
...
def main():
    ...
    try:
        while True:
            ...
            # bar_low 精度止损守护（tick级触发，实盘才启用）
            if tq_api is not None:
                try:
                    _check_auto_stop_lock(positions, tq_api, db_path, args.dry_run)
                    tq_api.wait_update(deadline=time.time() + 1.0)
                except Exception as e:
                    print(f"  [WARN] 守护tick失败: {e}")
                    time.sleep(1)
            else:
                time.sleep(1)

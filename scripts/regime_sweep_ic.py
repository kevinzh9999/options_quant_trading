#!/usr/bin/env python3
"""IC-specific regime filter sweep with better threshold mappings."""
    ...
    for adx_t in [15,20,25]:
        for vwap_t in [0.002,0.003,0.004]:
            ...
            for d,grp in bars.groupby("date"):
                ...
                ih=grp["high"].iloc[:6].max();il=grp["low"].iloc[:6].min();sc=[]
                for i in range(len(grp)):
                    p=grp.iloc[i]["close"];v=grp.iloc[i]["vwap"];a=grp.iloc[i]["adx"]
                    cA=p>ih or p<il;ap=grp.iloc[max(0,i-3)]["adx"]
                    cB=pd.notna(a) and a>adx_t and (pd.notna(ap) and a>ap)
                    vd=abs(p-v)/v if v>0 else 0;nc=True
                    if i>=5:
                        for j in range(max(0,i-4),i):
                            if (grp.iloc[j]["close"]-grp.iloc[j]["vwap"])*(grp.iloc[j+1]["close"]-grp.iloc[j+1]["vwap"])<0:nc=False;break
                    cC=vd>vwap_t and nc;sc.append(int(cA)+int(cB)+int(cC))
                dr[d.replace("-","")]=sc
...
def run_one_config(args):
    ...
    db=get_db();orig=sm.SignalGeneratorV2.score_all
    def _p(self,symbol,bar_5m,bar_15m,daily_bar,quote_data=None,
        ...
        if "datetime" in bar_5m.columns:dt=str(bar_5m.iloc[-1]["datetime"]);td=dt[:10].replace("-","")
        else:return r
        rs=regime_data.get(td,[])
        if not rs:return r
        today=bar_5m[bar_5m["datetime"].astype(str).str.startswith(dt[:10])]
        idx=min(len(today)-1,len(rs)-1);s=rs[idx] if idx>=0 else 0
        if r["total"]<thr_map.get(s,60):r["total"]=0;r["direction"]=""
        return r
    sm.SignalGeneratorV2.score_all=_p;pnl=0;n=0
    try:
        for td in dates:
        ...
        print(f"  Worker error: {sym} ADX{adx_t} VWAP{vwap_t} {thr_label}: {e}", flush=True)
...
if __name__ == "__main__":
    ...
    dates_df = db.query_df(
        ...
        "WHERE symbol='000905' AND period=300 AND d >= '2025-05-16' ORDER BY d"
    )
    ALL_DATES = [d.replace('-','') for d in dates_df['d'].tolist()]

    print(f"IC regime sweep (fine grid) | {len(ALL_DATES)} days | {cpu_count()} cores\n", flush=True)

    # IC uses 000905
    print("Precomputing IC regimes (000905)...", flush=True)
    t0 = _t.time()
    ...
    for td in ALL_DATES:
        ...
        bp += sum(x["pnl_pts"] for x in t if not x.get("partial"))
    print(f"  IC Baseline: {bp:+.0f}pt\n", flush=True)

    # IC-specific threshold maps: less conservative, more nuanced
    IC_THR = [
        # Format: (label, {0:red, 1:yellow, 2:green, 3:bright_green})
        # Current baseline thr=60, so red should be 60-65, not 70+
        ...
        ("60/60/55/50", {0:60, 1:60, 2:55, 3:50}),  # only lower when confident
        ("60/60/60/50", {0:60, 1:60, 2:60, 3:50}),  # only lower at 3 conditions
        ("60/60/60/55", {0:60, 1:60, 2:60, 3:55}),  # barely lower
    ]

    tasks = []
    for adx_t in [15, 20, 25]:
        for vwap_t in [0.002, 0.003, 0.004]:
            rd = ic_regimes[(adx_t, vwap_t)]
            for tl, tm in IC_THR:
                tasks.append(("IC", adx_t, vwap_t, tl, tm, rd, ALL_DATES))
    ...
    sr = sorted(results, key=lambda x: -x["pnl"])
    print(f"  ── IC Top 15 (baseline={bp:+.0f}pt) ──", flush=True)
    for i, r in enumerate(sr[:15]):
        delta = r["pnl"] - bp
        mk = " ★" if delta > 0 else ""
        print(f"    {i+1}. ADX>{r['adx']} VWAP>{r['vwap']*100:.1f}% thr={r['thr']}: "
              f"{r['pnl']:>+6.0f}pt {r['n']}笔 (Δ{delta:+.0f}){mk}", flush=True)

    print(f"\n  IC Bottom 3:", flush=True)
    for r in sr[-3:]:
        print(f"    ADX>{r['adx']} VWAP>{r['vwap']*100:.1f}% thr={r['thr']}: "
              f"{r['pnl']:>+6.0f}pt (Δ{r['pnl']-bp:+.0f})", flush=True)

    # How many configs beat baseline?
    better = sum(1 for r in results if r["pnl"] > bp)
    print(f"\n  优于baseline: {better}/{len(results)} ({better/len(results)*100:.0f}%)", flush=True)
